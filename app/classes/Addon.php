...
class Addon
{
        ...
        if ($this->checkAddonExistenceById($addon_id)) {
            if (databaseConnection()) {
                try {
                    ...
                    $result['user']['already_liked'] = $this->isRated($addon_id, $mb['user']['id']);
     ...
     * @return bool
     */
    public function isRated($addon_id, $user_id)
    {
        global $connection,$db_info;
        if (databaseConnection()) {
            try {
    ...
    {
        global $mb,$lang;
        $addon_already_liked = $this->isRated($_POST['id'], $mb['user']['id']);
