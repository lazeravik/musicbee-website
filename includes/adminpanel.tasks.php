...
require_once $siteRoot.'classes/Manager.php'; // Save and update the data on the database and json file
...
elseif (isset($_POST['save'])) {
	...
	if ($_POST['save'] == "stable") {
		...
		//load and use html purifier for the release notes.
		$config = HTMLPurifier_Config::createDefault();
		$config->set('HTML.Allowed', 'code,*[class],*[lang-rel],p,pre,table,thead,tbody,td,tr,th,h2,h1,h3,h4,h5,span,ul,li,ol');
		$def = $config->getHTMLDefinition(true);
		$def->addAttribute('code', 'lang-rel', 'Text');
		$purifier = new HTMLPurifier($config);
		$note_html = $purifier->purify($note_html_raw); //purify the release note html
		//$note_html =$note_html_raw; //purify the release note html
		/**
		* If Download link 1 and portable link is assigned while also this entry is not new! then we assume the admin is updating the CURRENT ACTIVE version and we want to update ALL the fields incl download link fields, other wise we ONLY want to update other fields than download links.*/
	...
	if (count(getVersionInfo($_POST['ver'], "byVersion"))==0 || isset($_POST['id_allversion'])) {
		if ($manager->saveArchieveData($note_html)) {
			if ($_POST['save'] == "stable") {
				echo '{"status": "1", "data": "'.$lang['AP_SR_SAVED_SUCCESS'].'"}';
...
//The client side delete button should be greyed out. BUT since the user HAS to be an admin we are not too strict on checking any fake delete request. THOUGH maybe in future there will be a check
elseif (isset($_POST['modify_type'])) {
	$manager = new Manager(); //create an instance of the MANAGER class
	if ($_POST['modify_type'] == "delete") {
		if($manager->deleteRecord($_POST['record_id'])){
			echo '{"status": "1", "data": "'.$lang['AP_RECORD_DELETED'].'"}';
		} else {
			echo '{"status": "0", "data": "'.$manager->errorMessage.'"}';
		}
	} elseif ($_POST['modify_type'] == "edit") {
		# code...
	}
}
...
function getAvavilability($id_version) {
	...
	if (databaseConnection()) {
		try {
			$sql = "SELECT available FROM ".SITE_MB_CURRENT_VERSION_TBL." WHERE ID_VERSION=".$id_version;
